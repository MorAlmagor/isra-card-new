...
const UpComing = ({ navigation, FavoriteMoviesArr }) => {
  const [movies, setMoviesData] = useState(false);
 
  if (movies === false) {
    axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=${theMovieDbKey}&language=en-US&page=1`)
      .then((res) => {
        if (movies === false) {
   ...
   return (
    <ScrollView>
      <View style={styles.container}>
        <Text style={styles.header}>Up Coming</Text>
        <View >
          <FlatList
            data={movies}
            horizontal={true}
...
const styles = StyleSheet.create({
  container: {
    ...
    paddingBottom: Device.Height * 0.03,
